...
def plot_wells_fractures_domain(well_labels, well_coords, is_hf,
    ...
    for iw in range(len(well_coords)):
        ...
        ax.plot([well_coo[0], well_coo[2]], [well_coo[1], well_coo[3]], linewidth=5, color=color, zorder=10)
        ...
        ax.scatter(X, Y, S, color='black', zorder=25)
        
        init_frac_coo = init_frac_coords[iw]
        [ax.plot([pfr_coord[0], pfr_coord[2]], [pfr_coord[1], pfr_coord[3]], 
                 linewidth=3, color='green', alpha=1, zorder=15, ls='-') for pfr_coord in frac_coords]
        [ax.plot([ifr_coord[0], ifr_coord[2]], [ifr_coord[1], ifr_coord[3]], 
        ...
        # well label
        if show_well_labels:
            ax.text(x_center+40, y_center+110*(well_coo[0]!=well_coo[2] or well_coo[1]!=well_coo[3]),
                    str(well_labels[iw]),
